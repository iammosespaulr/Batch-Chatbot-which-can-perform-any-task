...
void  moses_clock::alarm( moses_clock &c)
  {
   ...
   area = localtime(&t);
		  ...
		  cout<<asctime(area)<<"\r"<<" Countdown : "<<da;
        Sleep(1000);
        da=da-1;
      }
    for(i=0;i<4;i++)
    ...
    system("@echo off");
    system("echo Excecuting Program");
    speak("Excecuting Program");
    if(keyword=="happy new year"){
    	 system("Moses12.exe");
    }
    else if(keyword=="notepad")
    {
    	system("notepad");
    }
    else
    {cout<<"\nNo Programs Mentioned\n";
    speak("no programs mentioned");}
